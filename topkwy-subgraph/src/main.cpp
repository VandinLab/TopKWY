...
void runGaston() {
  ...
  for ( int i = 0; i < database.nodelabels.size (); i++ ) {
    if ( database.nodelabels[i].frequency >= minfreq &&
      ...
      //cout << "rungaston start with path of frequency " << database.nodelabels[i].frequency << endl;
      Path path ( i );
      //path.frequency = database.nodelabels[i].frequency;
      path.current_info.frequency = database.nodelabels[i].frequency;
      path.parent_info.frequency = -1;
      path.parent_info.max_as = -1;
      path.parent_info.min_as = -1;
      path.expand ();
    }
  }
}
...
int main ( int argc, char *argv[] ) {
  ...
  int n_perm = 1000; setK(-1); wy_processing_time = 0.0;

  char opt;
  while ( ( opt = getopt ( argc, argv, "m:a:j:f:r:tpa:i:c:o:wk:" ) ) != -1 ) {
    switch ( opt ) {
  ...
  if (!flag_in || !flag_class) {
   ...
   << "    -k [maximum number of (distinct) significant results]" << endl
	 << endl;
    return 1;
  }
  ...
  if (WYLIGHT) {
    cout << "Initialize random permutations ...";
  ...
  cout << endl << "Start GASTON to compute the corrected significance threshold" << endl;
  runGaston();
  if (WYLIGHT) {
  ...
  cout << "Runtime for WY computations (s):   \t" << wy_processing_time << endl;
  cout << "Total runtime (s):               \t" << ( (float) t3 - t1 ) / CLOCKS_PER_SEC << endl;
  cout << "Peak Memory (MB):                \t" << (int)measurePeakMemory() << endl;
  //statistics.print ();
