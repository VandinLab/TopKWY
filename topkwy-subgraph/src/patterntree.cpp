...
PatternTree::PatternTree ( Path &path, unsigned int legindex ) {
  ...
  #ifdef deepdebug
  cout << "call to PatternTree::PatternTree " << endl;
  #endif

  PathLeg &leg = (*path.legs[legindex]);
  //this->parent_frequency = path.frequency;
  //this->frequency = path.legs[legindex]->occurrences.frequency;
  this->current_info.frequency = path.legs[legindex]->occurrences.frequency;
  this->parent_info = path.current_info;

  // OUTPUT(path.legs[legindex]->occurrences.frequency)
  #ifdef deepdebug
  cout << "  processing tree of support " << this->current_info.frequency << endl;
  cout << "  support of parent path " << this->parent_info.frequency << endl;
  #endif
  checkTestable(path.legs[legindex]->occurrences.elements, path.legs[legindex]->occurrences.frequency,this->parent_info,this->current_info);
  ...
  #ifdef deepdebug
  cout << "call to PatternTree::PatternTree done " << endl;
  #endif
}
...
PatternTree::PatternTree ( PatternTree &parenttree, unsigned int legindex ) {
  ...
  //this->parent_frequency = parenttree.frequency;
  //this->frequency = parenttree.legs[legindex]->occurrences.frequency;
  this->current_info.frequency = parenttree.legs[legindex]->occurrences.frequency;
  this->parent_info = parenttree.current_info;
  ...
  cout << "  support of parent path " << this->parent_info.frequency << endl;
  #endif
  checkTestable(parenttree.legs[legindex]->occurrences.elements, parenttree.legs[legindex]->occurrences.frequency,this->parent_info,this->current_info);
...
void PatternTree::expand () {
  ...
  if ( phase > 2 )
    for ( int i = 0; i < closelegs.size(); i++ ) {
      ...
      p.parent_info = this->current_info;
      p.expand ();
