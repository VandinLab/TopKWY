...
Path::Path ( Path &parentpath, unsigned int legindex ) {
  ...
  //cout << "  here 1" << endl;
  //this->frequency = parentpath.legs[legindex]->occurrences.frequency;
  this->current_info.frequency = parentpath.legs[legindex]->occurrences.frequency;
  #ifdef deepdebug
  cout << "  with frequency " << this->current_info.frequency << endl;
  #endif
  //this->parent_frequency = parentpath.frequency;
  this->parent_info = parentpath.current_info;
  //cout << "  here 2" << endl;
  #ifdef deepdebug
  cout << "  processing path of support " << parentpath.legs[legindex]->occurrences.frequency << endl;
  cout << "  support of parent path = " << this->parent_info.frequency << endl;
  #endif
  checkTestable(parentpath.legs[legindex]->occurrences.elements, parentpath.legs[legindex]->occurrences.frequency, this->parent_info, this->current_info);
  ...
  #ifdef deepdebug
  cout << "call to Path::Path done " << endl;
  #endif
}
...
void Path::expand2 () {
    ...
    #ifdef deepdebug
    cout << "call to Path::expand2 " << endl;
    cout << "  with frequency " << this->current_info.frequency << endl;
    #endif
  ...
  if ( closelegs.size () && phase > 2 ) {
    ...
    for ( int i = 0; i < closelegs.size (); i++ ) {
      if ( closelegs[i]->tuple.from == from &&
        ...
        #ifdef deepdebug
        cout << "  processing CL path of support " << closelegs[i]->occurrences.frequency << endl;
        cout << "  support of parent path = " << this->current_info.frequency << endl; // correct
        #endif
        // OUTPUT(closelegs[i]->occurrences.frequency)
        bounds_info current_info_temp;
        current_info_temp.frequency = closelegs[i]->occurrences.frequency;
	      checkTestableCl(closelegs[i]->occurrences.elements, closelegs[i]->occurrences.frequency, this->current_info , current_info_temp);
        #ifdef deepdebug
        cout << "  done processing CL path of support " << closelegs[i]->occurrences.frequency << endl;
        #endif
  ...
  statistics.patternsize--;
  #ifdef deepdebug
  cout << "call to Path::expand2 done " << endl;
  #endif
}
...
void Path::expand () {
  ...
  #ifdef deepdebug
  cout << "call to Path::expand done " << endl;
  #endif
}
