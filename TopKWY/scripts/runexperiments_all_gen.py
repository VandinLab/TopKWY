...
K_ = [1000000]
G_ = [1 , 2 , 5 , 10 , 100 , 1000]

for G in G_:
	for K in K_:

		datasets = ['covtype','retail','mushroom','bms-web1','bms-web2','cod-rna',
		'breast-cancer','ijcnn1','a9a','svmguide3','T10I4D100K'];
		...
		for i in datasets:
			...
			runs_count[i] = 0
			sign_patterns[i] = np.zeros(runs)
			deltas[i] = np.zeros(runs)


		def launch_experiment(dat_name , K , jp , alpha , flag , genFWER, run_id):
			print "Run experiment "+str(run_id+1)+" for " + dat_name
			print "K = "+str(K)+" jp = "+str(jp)+" alpha = "+str(alpha)+" genFWER = "+str(genFWER)+" flag = "+str(flag)
			...
			out_file_name = 'experiment_'+flag+'_run'+str(run_id)+"_a"+str(alpha)+"_j"+str(jp)+'_k'+str(K)+'_g'+str(genFWER)+'.txt';
			cmd = "cd datasets/" + dat_name + "/ && ./"+topkwy_name+" -s " + dat_name + "_new.spec -k "+str(K)+" -j "+str(jp)+" -a "+str(alpha)+" -r "+str(maxram)+" -g "+str(genFWER)+"  > "+out_file_name+" 2>&1 &"
			os.system(cmd)
			#print cmd
		...
		if len(flag) > 0:
			out_file_name = 'experiment_'+flag+'_run'+str(0)+"_a"+str(alpha)+"_j"+str(jp)+'_k'+str(K)+'_g'+str(G)+'.txt';
		...
		for dat_name in datasets:
			launch_experiment(dat_name , K , jp , alpha , flag , G , 0)
		...
		while (running == 1):
			...
			for dat_name in running_datasets[:]:
				...
				out_file_name = 'experiment_'+flag+'_run'+str(runs_count[dat_name])+"_a"+str(alpha)+"_j"+str(jp)+'_k'+str(K)+'_g'+str(G)+'.txt';
				...
				while (ok==1) :
					...
					if len(line) > 0:
						...
						if searchObj:
							tested_txt = searchObj.group()
							tested_txt = tested_txt[32:]

							numofpattern = int(tested_txt)
							results = sign_patterns[dat_name]
							results[runs_count[dat_name]] = numofpattern
							sign_patterns[dat_name] = results

							print "  Significant patterns: " + str(numofpattern)

						searchObj = re.match( r'Corrected significance threshold: .*', line, re.M|re.I)
						if searchObj:
							tested_txt = searchObj.group()
							tested_txt = tested_txt[34:]

							delta_ = float(tested_txt)
							results = deltas[dat_name]
							results[runs_count[dat_name]] = delta_
							deltas[dat_name] = results

							print "  Corrected Thr.: " + str(delta_)
			...
			for i in datasets:
				if i not in running_datasets:
					...
					if runs_count[i] < runs:
						time.sleep(randint(10, 20))
						launch_experiment(i , K , jp , alpha , flag , G , runs_count[i])
						running_datasets.append(i)

						# print results of this run on file
						ri = runs_count[i]-1
						file_out_log = open(flag+"_"+str(K)+"_"+str(jp)+".txt",'a')
						file_out_log.write(str(K)+";"+str(jp)+";"+str(alpha)+";"+str(G)+";"+i+";"+str(times[i][ri])+";"+str(memory[i][ri])+";"+str(tested[i][ri])+";"+str(deltas[i][ri])+";"+str(sign_patterns[i][ri])+";"+str(ri)+";\n")
						file_out_log.close()
						...
						all_results_file.write(str(K)+";"+str(jp)+";"+str(alpha)+";"+str(G)+";"+i+";"+str(mean_time)+";"+str(deltas[i][0])+";"+str(sign_patterns[i][0])+";"+str(variance_time)+";"+str(mean_memory)+";"+str(variance_memory)+";\n")
						datasets.remove(i)
