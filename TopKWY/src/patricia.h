...
int readSpecFile(int argc, char *argv[]){
	...
	char opt;
  while ( ( opt = getopt ( argc, argv, "m:j:s:r:k:a:g:" ) ) != -1 ) {
    switch ( opt ) {
    ...
    case 'k': K_significant_patterns = atoi(optarg); break;
		...
		case 'g': generalized_FWER = atoi(optarg); break;
    }
  }
	...
	if(!spec_file_given){
		...
		<< "    -g [generalized FWER (default: 1)]" << endl
...
void firstScan(PatriciaTrie* p , bool* permutations){
	...
	cout << "effect_num_tr " << effect_num_tr << endl;
