...
double *min_pval;
// Minimum P-values for generalized FWER
std::vector< priority_queue<double> > min_gen_pvalues;
...
void update_delta(){
	...
	bool queue_not_empty = alpha_quantile.size() > 0;
	bool not_updated_delta = alpha_quantile.top().p_value == delta;
	bool FWER_not_controlled = (double)below_delta / (double)jp > alpha;
	...
	while(queue_not_empty && (FWER_not_controlled || not_updated_delta)){
		...
		alpha_quantile.pop();
		queue_not_empty = alpha_quantile.size() > 0;
		if(queue_not_empty){
			update_queue_values();
			not_updated_delta = alpha_quantile.top().p_value == delta;
		}
		else{
			cout << " alpha-quantile queue got empty! " << endl;
		...
		--below_delta;
		FWER_not_controlled = (double)below_delta / (double)jp > alpha;
...
double* computePValues(int x_S , int* a_S){
	...
	for(int aj=0; aj<jp+1; aj++){
		...
		// fill queue of minimum p-values
		if(aj > 0 && to_return[aj] <= delta){
			int aj_index = aj-1;
			// insert a new minimum p-value if there are less than generalized_FWER or
			// if the new p-value is inferior to the current maximum p-value stored in the queue
			if(min_gen_pvalues[aj_index].size() < generalized_FWER || to_return[aj] < min_gen_pvalues[aj_index].top()){
				priority_queue<double>& current_queue = min_gen_pvalues[aj_index];
				//cout << "inserting new minimum p-value "<< to_return[aj] <<" in the queue for index " << aj_index << endl;
				//cout << "delta " << delta << endl;
				current_queue.push(to_return[aj]);
				//cout << "  new size " << current_queue.size() << endl;
				// in case the queue is larger than generalized_FWER, remove the highest element
				while(current_queue.size() > generalized_FWER){
					//cout << "  permutation of index " << aj_index << endl;
					//cout << "  size of queue " << current_queue.size() << endl;
					//cout << "  removing p-value " << current_queue.top() << endl;
					current_queue.pop();
					//cout << "  new highest p-value " << current_queue.top() << endl;
				}
				// update the min-max p-value of the permutation
				if(current_queue.size() == generalized_FWER && min_pval[aj] > current_queue.top()){
					min_pval[aj] = current_queue.top();
					//cout << "  min p-value updated to " << min_pval[aj] << " for index " << aj_index << endl;
				}
				if(quantile_indexes[aj] == false && min_pval[aj] <= delta){
					// add this permutation to the alpha-quantile
					temp_alpha_item.p_value = min_pval[aj];
					...
					below_delta++;
					//cout << "  below_delta increased to " << below_delta << endl;
...
void printMinPvaluesGFWER(){

	ofstream out_file;
	out_file.open ("minpvalsgfwer.txt");
	for(int aj_index=0; aj_index<min_gen_pvalues.size(); aj_index++){
		out_file << aj_index << ";";
		priority_queue<double>& current_queue = min_gen_pvalues[aj_index];
		while(current_queue.size() > 0){
			double min_pval_ = current_queue.top();
			current_queue.pop();
			out_file << min_pval_ << ";";
		}
		out_file << endl;
