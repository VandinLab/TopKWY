...
typedef QueueMinMax_results_std* typeQueueRes;

double max_rejected_p_value;
  ...
  if(perm_supp.size() < jp+1)
    ...
    cout << "HT[i].intersection[0] = " << HT[i].intersection[0] << endl;*/
      ...
      //cout << "tmpIts generated! tmpIts->support " << tmpIts->support << " tmpIts->info->a_S_ " << tmpIts->info->a_S_ << endl;
			      ...
			      bool can_be_significant = getLogPsiExact(tmpIts->support) <= log_delta;
			      if(can_be_significant){
			      double log_p_value = computeLogPValue(tmpIts->support , tmpIts->info->a_S_);
			      if( store_results && log_p_value <= log_delta ){
              ...
              /* Top-K strategy */
              if(K_significant_patterns > 0 && q_res->getKElements() >= K_significant_patterns && log_delta > q_res->observed_p_values.top()){
                ...
                cout << " sigma " << suppMin;
                cout << " delta " << delta;
                cout << " getLogPsi(sigma) " << getLogPsi(suppMin);
                cout << " getLogPsi(sigma-1) " << getLogPsi(suppMin-1);
                cout << " q_res->getKElements() " << q_res->getKElements();
                cout << " q_res->observed_p_values.top() " << q_res->observed_p_values.top();
                cout << " log_delta " << log_delta << endl;*/

                //cout << "Top-K strategy update! " << endl;
                ...
                while(getLogPsi(suppMin) > log_delta){
                  ++suppMin;
                }
                suppMinCurr = suppMin;
                q_res->removeMax(log_delta);
  ...
  prune_current_ppce = false;
	...
	for (int i=father->info->coreIndex-1; i>=0; i--){
		...
		if (HT[i].count>=suppMin){
          ...
          #ifdef enable_bounds
          can_be_significant = getLogPsiExact(ppce_support) <= log_delta;
          #endif
...
int main(int argc, char **argv){
  ...
  cout << "before firstscan suppmin is "<< suppMin << endl;
...
//			cloempty=cloempty+" ";
  ...
  ssoutput_path_result << fileinput << "_k" << K_significant_patterns << "_a" << alpha<< "_j" << jp <<"_g" << generalized_FWER << "_result.txt";
  ...
  cout << "before secondscan suppmin is "<< suppMin << endl;
  ...
  int suppMin_temp = suppMin;

	p->secondScan_new();

  suppMin = suppMin_temp;

  cout << "after secondscan suppmin is "<< suppMin << endl;
  ...
  long long bigmem=(long long)totItem*(long long)100;
  cout << "totItem: " << totItem << endl;
  cout << "bigmem: " << bigmem << endl;
  ...
  //p->printCountDistribution();
	...
	typeQueueRes q_res=new QueueMinMax_results_std();
  ...
  cout << "before findExtCloEmpty suppmin is "<< suppMin << endl;
  ...
  cout << "Memory peak after findExtCloEmpty: " << (int)measurePeakMemory() << " MByte" << endl;
	...
	cout << "Max allowed memory: " << (double)max_ram / 1000.0 << " GB" << endl;
  ...
  // PATTERN EXPLORATION BEGINS
    ...
    cout << "suppMin = " << suppMin << endl;*/
      ...
      cout << "   patterns observed: " << q_res->observed_p_values.size() << endl;
		  cout << "   q_res->getLogMax() " << q_res->getLogMax() << endl;
      cout << "   q_res->getMax() " << pow(10.0 , q_res->getLogMax()) << endl;
		  cout << "   q_res->getLogMin() " << q_res->getLogMin() << endl;
      cout << "   q_res->getMin() " << pow(10.0 , q_res->getLogMin()) << endl;
    ...
    //cout << "max extracted! max->support " << max->support << " max->info->a_S_ " << max->info->a_S_ << endl;
		if(store_results && (max->info->p_value < delta || (delta == 0.0 && computeLogPValue(max->support , max->info->a_S_) <= log_delta ))){
    ...
    if(q_res->getInqueue() > 0 && q_res->getLogMin() < getLogPsi(s_supp)){
      ...
      std::priority_queue<ResultPattern*, std::vector<ResultPattern*>, Compare_Results_topsmall> res_queue_;
      std::vector< ResultPattern* > results_;
      results_ = q_res->removeMin(getLogPsi(s_supp));
      for(int i =0; i<results_.size(); i++){
        res_queue_.push(results_[i]);
      }
      /*while(q_res->getInqueue() > 0 && q_res->getLogMin() < getLogPsi(s_supp)){
        res_queue_.push(q_res->removeMin(log_delta));
      }*/
      ...
      while(res_queue_.size() > 0){
        ...
        cout << "getLogPsi(s_supp) " << getLogPsi(s_supp) << endl;
        cout << "log_delta " << log_delta << endl;*/
        if(toOutput_pattern->log_p_value > log_delta){
          cout << "output result with log_p_value " << toOutput_pattern->log_p_value << endl;
          cout << "getLogPsi(s_supp) " << getLogPsi(s_supp) << endl;
          cout << "log_delta " << log_delta << endl;
          abort();
        ...
        max_rejected_p_value = (toOutput_pattern->p_value > max_rejected_p_value) ? toOutput_pattern->p_value : max_rejected_p_value;
    ...
    q_res->removeMax(log_delta);
		...
		//while(q_res->getMax() > -0.5 && q_res->getMax() > getPsi(suppMin) && q_res->getMin() <= getPsi(suppMin)){
    /*int debug_msg_out = 0;
    if(q_res->getInqueue() > 0 && q_res->getMax() > getPsi(suppMin-1)){
      cout << " removing infrequent patterns " << endl;
      debug_msg_out = 1;
    }*/
		/*while(q_res->getInqueue() > 0 && q_res->getMax() > getPsi(suppMin-1)){*/
      /*cout << " -------------------------- " << endl;
      ...
      cout << "getLogPsi(suppMin-1) " << getLogPsi(suppMin-1) << endl;
      cout << "getLogPsi(suppMin) " << getLogPsi(suppMin) << endl;*/
      /*cout << "removing q_res->getInqueue() " << q_res->getInqueue() << " , q_res->getMax() " << q_res->getMax() << endl;
			q_res->removeMax();*/
      /*cout << " -------------------------- " << endl;*/
			//cout << " remove max done! " << endl;
			//q_res->printLowestHeapLevel();
      //cout << "new max p-value is now " << q_res->getMax() << endl;
		/*}*/
    /*if(debug_msg_out == 1){
      cout << " done removing infrequent patterns " << endl;
    }*/
  ...
  //cout << " q_res->observed_p_values.size() " << q_res->observed_p_values.size() << endl;
  //cout << " highest p-value in res queue " << pow(10.0 , q_res->observed_p_values.top()) << endl;
  //cout << " highest log-p-value in res queue " << q_res->observed_p_values.top() << endl;
		...
		// output all itemset in the queue which are surely significant
		// delta is now exact and can be used to flag itemsets as significant
    if(q_res->getInqueue() > 0){

      // priority queue to sort the last results
      std::priority_queue<ResultPattern*, std::vector<ResultPattern*>, Compare_Results_topsmall> res_queue_;
      std::vector< ResultPattern* > results_ = q_res->removeMin(log_delta);
      for(int i=0; i<results_.size(); i++){
        res_queue_.push(results_[i]);
      }
      ...
      //cout << "log_delta " << log_delta << endl;
      while(res_queue_.size() > 0 && (res_queue_.top()->log_p_value <= log_delta || res_queue_.top()->p_value <= delta)){
        toOutput_pattern = res_queue_.top();
        res_queue_.pop();
        //cout << "output result with log_p_value " << toOutput_pattern->log_p_value << endl;
        //cout << "log_delta " << log_delta << endl;
      ...
      /*if(res_queue_.size() == 0){
        cout << "res_queue_ is now EMPTY " << endl;
      }
      else{
        cout << "highest p-value still in res_queue_ " << res_queue_.top()->log_p_value << endl;
      }*/
  ...
  cout << "Highest rejected p-value " <<  max_rejected_p_value << endl;
