 ...
 */
 ...
 #ifndef _queueheap_h_
 #define _queueheap_h_
...
class Compare_Results_topsmall
{
public:
...
class Compare_Results_toplarge
{
public:
    bool operator() (ResultPattern* a, ResultPattern* b)
    {
...
///class for the min-max-based priority queue for results
class QueueMinMax_results_std{
		public:
		void insert(ResultPattern* its);	///insert the ResultPattern its in the heap
		void removeMax(double log_delta);		///remove the element with higher p-value from the queue
		std::vector< ResultPattern* > removeMin(double log_delta);		///remove the element with smaller p-value from the queue
		QueueMinMax_results_std();			///default constructor
		tipoInt getInqueue();   // returns number of elements in the queue
		tipoInt getKElements();
		void insert_observed(double log_observed_pval);
		int getLogMax();
		double getLogMin();
		void printElements();
		std::priority_queue< ResultPattern*, std::vector<ResultPattern*>, Compare_Results_toplarge > results_p_values;
		std::priority_queue<double> observed_p_values;
	private:
		double q_min_pvalue; // minimum p-value stored in the queue
};
